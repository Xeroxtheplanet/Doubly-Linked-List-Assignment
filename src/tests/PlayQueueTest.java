...
public class PlayQueueTest {
    ...
    void testShuffle() {
        ...
        assertEquals(testSongsA[4], smallShuffled7.start.next.next.next.next.next.song); //Complete with the NEXT songs in their original order. WHY WOULD IT NOT SAY TO REMOVE THE ONE WHERE A CYCLE OCCURS
        assertEquals(testSongsA[6], smallShuffled7.start.next.next.next.next.next.next.song); //Now works, says this doesn't match up (this is because a lookup list is required, can't insert ones that have already been inserted
